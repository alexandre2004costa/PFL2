...
game_over([_, [FirstLine | Board], _, _, _, _, _, 1, _], Winner) :- % Checks for a win condition for both players, Board 5x5
    ...
    verify_black_win(1, Visited2, BlackWin), !,

    decide_winner(WhiteWin, BlackWin, Winner).

game_over([_, [FirstLine | Board],  _, _, _, _, _, 2, _], Winner) :- % Checks for a win condition for both players, Board 4x4
    ...
    dfs([FirstLine | Board], 'B', Stack2, [], Visited2), !, 
    verify_black_win(2, Visited2, BlackWin), !,
